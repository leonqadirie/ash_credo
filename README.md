...
```bash
mix igniter.install ash_credo@0.18.0
```
...
def deps do
  [
    {:ash_credo, "~> 0.18.0", only: [:dev, :test], runtime: false}
  ]
end
```
...
| `AnonymousFunctionInDsl` | Refactor | Normal | No | Flags `fn` and local `&` captures anywhere in the resource DSL, and remote captures where `change`/`validate`/`prepare`/`calculate` wrap them. Spark lifts each one into a generated function on the resource. Anonymous changes and validations can never be atomic, and anonymous calculations can never supply an expression, so extract them into callback modules |
