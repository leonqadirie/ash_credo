...
| `AnonymousFunctionInDsl` | Refactor | Normal | No | Flags `fn`/`&` captures anywhere in the resource DSL: `change`/`validate`/`prepare`/`calculate`, `run`, pub/sub `transform` and extension options alike. Spark lifts each one into a generated function on the resource, and those on changes and validations can never be atomic, so extract them into callback modules |
